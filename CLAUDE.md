...
`npm test` runs `test/parser.ts` (a `tape`/`fast-check` property test round-tripping the condition parser's AST through stringify/parse) — this used to be a stub (`echo ... && exit 1`) even though `tape`/`fast-check` were already installed; fixed so `npm install && npm test` actually verifies something. `test/race.ts` (a similar property test over `RaceSolver`, parameterized via `--runs`/`--timestep`) is **not** wired into `npm test` — it currently fails to type-check (`RaceParams` shape mismatch, 2 errors, part of the pre-existing `test/`/`tools/` gap above) — run it manually with `ts-node test/race.ts --runs <n>` once that's fixed, not before. `test/bench/bench.ts` and `test/regression/` (a checkpoint-based regression system — `create-checkpoint.ts`/`check.ts`/`knowncases.ts`) are also standalone, invoked directly via `ts-node`, not part of `npm test`.
