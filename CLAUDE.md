...
4. **Pacer skill triggers are sampled once per race slot, not once per scenario.** `RaceSolverBuilder.buildPacer` used to call `setupPacerSkillTriggers` unconditionally on every invocation — regenerating the pacer's entire trigger table from scratch per scenario, an O(n²) cost as sample count grows. `prepPacerTriggers(pacerSlots, baseSeed)` now samples each slot's table once, called before the scenario loop starts (`mackoz/uma-tools`'s `umalator/compare.ts` is the one caller); `buildPacer` just indexes into `_pacerTriggersBySlot[slot][sdi][i % length]`. This is a genuine numeric-output change for any race with a virtual pacemaker — scenario `i` now takes trigger index `i` of one fixed stream instead of a fresh per-scenario stream, matching how the main horses' own triggers already worked (`_skillTriggers` via `deriveSeed`). If you're diffing simulator output against a pre-`0861c8f` run and a virtual pacemaker is involved, expect a difference; that's this fix, not a regression.
