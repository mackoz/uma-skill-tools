...
`RaceSolverBuilder.ts`'s `conditionsWithActivateCountsAsRandom` (see ADR-0010) shadows it, along
with the six `activate_count_*` names, with an unconditionally-satisfied stand-in for callers that
opt into `withActivateCountsAsRandom()` — that shadow only applies when a caller explicitly asks
for it (uma-tools' Course Chart, and the `basinnhyou.ts` CLI tool it's descended from); the real
