...
| time 3, 7 | `MultiplyRemainHp` | bracketed on HP at activation, SKL-7. Only meaningful under `mode: 'compare'`, the one mode that gets a real `GameHpPolicy`; every other path gets `NoopHpPolicy`, whose `hpRemaining()` is `Infinity`, so duration scaling saturates at its top bracket there — see `docs/adr/0013-value-scaling-identity-fallthrough.md` |
