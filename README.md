...
Three condition names remain unregistered and will still throw the new named `ParseError` — `temptation_opponent_count_behind`/`temptation_opponent_count_infront` and `is_other_character_activate_advantage_skill` — along with roughly a dozen JP-only names (`furlong`, `is_abroad`, `run_at_full_speed_random`, etc.). See `mackoz/uma-tools`'s `plans/engine-comparison/skills.md` for the full list and which shipped skills each one affects.
