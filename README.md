...
Fourteen condition names used by shipped skills remain unregistered and will still throw the new named `ParseError`: three that ship on Global too — `temptation_opponent_count_behind`/`temptation_opponent_count_infront` and `is_other_character_activate_advantage_skill` — plus eleven JP-only names (`fan_count`, `furlong`, `is_abroad`, `is_activate_heal_skill`, `is_exist_skill_id`, `near_infront_count`, `phase_first_half_straight_random`, `phase_laterhalf`, `phase_latter_half_straight_random`, `run_at_full_speed_random`, `up_slope_random_later_half`).
