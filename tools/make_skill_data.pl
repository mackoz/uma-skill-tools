...
my $skills = {};
while ($select->fetch) {
	my @effects_1 = ({type => $ability_type_1_1, modifier => patch_modifier($id, $float_ability_value_1_1), target => $target_type_1_1, valueUsage => $ability_value_usage_1_1});
	if ($ability_type_1_2 != 0) {
		push @effects_1, {type => $ability_type_1_2, modifier => patch_modifier($id, $float_ability_value_1_2), target => $target_type_1_2, valueUsage => $ability_value_usage_1_2};
	}
	if ($ability_type_1_3 != 0) {
		push @effects_1, {type => $ability_type_1_3, modifier => patch_modifier($id, $float_ability_value_1_3), target => $target_type_1_3, valueUsage => $ability_value_usage_1_3};
	}
	my @triggers = ({
	...
	if ($condition_2 ne '' && $condition_2 ne '0') {
		...
		if ($ability_type_2_2 != 0) {
			push @effects_2, {type => $ability_type_2_2, modifier => patch_modifier($id, $float_ability_value_2_2), target => $target_type_2_2, valueUsage => $ability_value_usage_2_2};
		}
		if ($ability_type_2_3 != 0) {
			push @effects_2, {type => $ability_type_2_3, modifier => patch_modifier($id, $float_ability_value_2_3), target => $target_type_2_3, valueUsage => $ability_value_usage_2_3};
		}
		push @triggers, {
