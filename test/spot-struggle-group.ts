...
test('entry is measured from the frontmost uma, not symmetrically around the caller', t => {
	...
	t.notEqual(a.leadCompetitionStart, null, 'frontmost uma joins');
	t.notEqual(b.leadCompetitionStart, null, 'caller, 3m behind frontmost, joins');
	t.equal(c.leadCompetitionStart, null, '4.5m behind frontmost stays out, despite being close to the caller');
	t.end();
});

test('Oonige uses the 3.75m DistanceGap1, not a wider 5m entry', t => {
	const noTrigger = field(makeUma(200.0, Strategy.Oonige), makeUma(204.1, Strategy.Oonige)); // 4.1m apart
	tryStart(noTrigger[0]);
	t.ok(noTrigger.every(u => u.leadCompetitionStart === null), '4.1m apart never triggers (would have under the old 5m rule)');

	const triggers = field(makeUma(200.0, Strategy.Oonige), makeUma(203.1, Strategy.Oonige)); // 3.1m apart
	tryStart(triggers[0]);
	t.ok(triggers.every(u => u.leadCompetitionStart !== null), '3.1m apart triggers');
	t.end();
});

test('LaneGap1 blocks entry laterally', t => {
	const blocked = field(makeUma(200, Strategy.Nige, {lane: 0}), makeUma(201.5, Strategy.Nige, {lane: 2.0}));
	tryStart(blocked[0]);
	t.ok(blocked.every(u => u.leadCompetitionStart === null), '2.0m lane gap (> 0.165*11.25 = 1.856m) blocks entry');

	const allowed = field(makeUma(200, Strategy.Nige, {lane: 0}), makeUma(201.5, Strategy.Nige, {lane: 1.8}));
	tryStart(allowed[0]);
	t.ok(allowed.every(u => u.leadCompetitionStart !== null), '1.8m lane gap (< 1.856m) allows entry');
	t.end();
});

test('lateral entry check is skipped when lane movement is disabled', t => {
	const umas = field(
		makeUma(200, Strategy.Nige, {lane: 0, laneMovement: false}),
		makeUma(201.5, Strategy.Nige, {lane: 2.0, laneMovement: false}),
	);
	tryStart(umas[0]);
	t.ok(umas.every(u => u.leadCompetitionStart !== null), 'a 2.0m lane gap is ignored with lane movement off (e.g. the Skill Chart)');
	t.end();
});

test('the entry window is section 6 (sectionLength * 6), not section 5', t => {
	...
	tryStart(triggers[0]);
	t.ok(triggers.every(u => u.leadCompetitionStart !== null), 'triggers because 599.2 <= sectionLength*6 (600)');

	const noTrigger = field(makeUma(604, Strategy.Nige), makeUma(601, Strategy.Nige));
	tryStart(noTrigger[0]);
	t.ok(noTrigger.every(u => u.leadCompetitionStart === null), 'neither uma is within section 6 (601 > 600) -- no trigger');
	t.end();
});

test('CheckStartDistance (150m) unlocks spot struggle field-globally, not per-uma', t => {
	const tooEarly = field(makeUma(137, Strategy.Nige), makeUma(135, Strategy.Nige));
	tryStart(tooEarly[0]);
	t.ok(tooEarly.every(u => u.leadCompetitionStart === null), 'neither uma has reached 150m -- no trigger');
	...
	t.ok(unlockedByOther[0].leadCompetitionStart !== null && unlockedByOther[1].leadCompetitionStart !== null,
		'a third uma (any style) past 150m unlocks the field-wide check, so the 137/135 pair triggers early');
	t.end();
});

test('EndSection (9) is an absolute position shared by the group, not an offset from each trigger point', t => {
	...
	t.equal(umas[0].leadCompetitionEnd, expectedEnd, `frontmost uma's cap is the absolute ${expectedEnd}`);
	t.equal(umas[1].leadCompetitionEnd, expectedEnd, `other member's cap is the same absolute ${expectedEnd}, not pos-relative`);
	t.end();
});

test('NigeCount/OonigeCount: 1 -- each style triggers once per race, independently of the other style', t => {
	...
	tryStart(nige4);
	t.ok(nige3.leadCompetitionStart !== null && nige4.leadCompetitionStart !== null, 'the near pair (nige3/nige4) triggers');
	t.ok(nige1.leadCompetitionStart === null && nige2.leadCompetitionStart === null, 'the far pair is left out of that group');

	tryStart(nige1); // Nige already triggered this race -- must be a no-op regardless of nige1's own position
	t.ok(nige1.leadCompetitionStart === null, 'Nige style already used its one-per-race budget; no second group forms');

	tryStart(oonige1); // a completely separate style, unaffected by Nige's trigger
	t.ok(oonige1.leadCompetitionStart !== null && oonige2.leadCompetitionStart !== null, 'Oonige triggers independently of Nige');
	t.end();
});
...
test('distance exit (DistanceGap2) requires clearing every active participant, not just the nearest', t => {
	...
	exit(z);
	t.ok(z.leadCompetition, 'stays active: not yet >=5m behind every active participant (only x, not y)');

	z.pos = 390; // now 10m behind x and 6m behind y
	exit(z);
	t.notOk(z.leadCompetition, 'exits once >=5m behind ALL active participants');
	t.ok(z.leadCompetitionDistanceExited, 'flagged as a distance exit, for the cascade rule');
	t.equal(z.leadCompetitionEnd, 390, 'leadCompetitionEnd records where she actually left');
	t.end();
});
...
test('lateral exit (LaneGap2)', t => {
	...
	t.ok(z1.leadCompetitionDistanceExited);
...
test('cascade: the last struggler exits once every other participant left via the distance/lateral exit', t => {
	...
	exit(a);
	t.notOk(a.leadCompetition, 'the last active struggler exits once everyone else has distance-exited');
	t.ok(a.leadCompetitionDistanceExited);
	t.end();
});
...
test('cascade does not fire from natural duration/position-cap expiry', t => {
	...
	exit(a);
	t.ok(a.leadCompetition, 'stays active: the one other participant did not leave via a distance exit');
	t.end();
});
...
test('the section-9 cap exit is never misreported as a distance exit', t => {
	...
	exit(a);
	t.notOk(a.leadCompetition, 'exits via the position cap');
	t.notOk(a.leadCompetitionDistanceExited, 'NOT flagged as a distance exit -- the cap check returns before that logic runs');
	t.equal(a.leadCompetitionEnd, 800);
	t.end();
});
